...
int main(int argc, char** argv) {
    ...
    ws.start();
    for (int i = 0; i < 500 && !open; ++i) std::this_thread::sleep_for(10ms);
    if (!open) { std::fprintf(stderr, "cannot connect to port %d\n", port); return 3; }

    ws.sendText(R"({"type":"hello","version":1,"sampleRate":16000,"channels":1,"format":"s16le","streams":["mic","tab"]})");
