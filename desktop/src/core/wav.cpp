...
std::optional<WavData> readWavPcm16Mono(const std::string& path, std::string& error) {
    ...
    if (!f) { error = "cannot open " + path; return std::nullopt; }
    char riff[4], wave[4];
    uint32_t riffSize = 0;
    f.read(riff, 4); f.read(reinterpret_cast<char*>(&riffSize), 4); f.read(wave, 4);
    if (!f || std::strncmp(riff, "RIFF", 4) != 0 || std::strncmp(wave, "WAVE", 4) != 0) {
        error = "not a RIFF/WAVE file"; return std::nullopt;
    }
    WavData out;
